...
public class QueryTest {
    ...
    void dynamicPaginationLastPage() {
        ...
        // calc page
        int lastPageNum = (int) Math.ceil(numEmployees / (double) pageSize); // Total de paginas

        // pagination
        String empJpql = "select e from Employee e";
        Query<Employee> query = session.createQuery(empJpql, Employee.class);
        int position = (lastPageNum - 1) * pageSize; // Ultima pagina
        query.setFirstResult(position); // Donde empezamos
        query.setMaxResults(pageSize); // Cuantos registros
